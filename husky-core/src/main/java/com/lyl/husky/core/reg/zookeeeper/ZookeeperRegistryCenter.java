package com.lyl.husky.core.reg.zookeeeper;
...
public final class ZookeeperRegistryCenter implements CoordinatorRegistryCenter {
    ...
    /**
     * TODO 等待500ms，cache先关闭在关闭client，否则会抛出异常
     * 因为一部处理，可能导致client先关闭而cache还未关闭结束
     * 等待Curator新版本解决这个bug
    ...
    public void addCacheData(final String cachePath) {
        ...
        caches.put(cachePath.concat("/"), cache);
    }

    @Override
    public void evictCacheData(String cachePath) {
        TreeCache cache = caches.remove(cachePath.concat("/"));
        if (null != cache){
            cache.close();
        }
    }
    ...
    public Object getRawCache(final String cachePath) {
        return caches.get(cachePath.concat("/"));
    }
